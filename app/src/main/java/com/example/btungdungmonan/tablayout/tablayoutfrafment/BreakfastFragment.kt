...
import com.example.btungdungmonan.databinding.FragmentBreakfastBinding
...
class BreakfastFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        homeViewModel.getBreakfastApi()
        getDessert()
    }
    private fun getDessert() {
        var listParetChil1:MutableList<ListDataParentChild> = mutableListOf()
        var parentBreakfast:MutableList<CategoriMeal> = mutableListOf()
        homeViewModel.ldtCategoryMealDaTa().observe(viewLifecycleOwner, Observer {listCategoriMeal->
            Log.d("AAAAAABBBBB",listCategoriMeal[12].strCategory)
            parentBreakfast.add(CategoriMeal("${listCategoriMeal[12].idCategory}","${listCategoriMeal[12].strCategory}","${listCategoriMeal[12].strCategoryDescription}","${listCategoriMeal[12].strCategoryThumb}"))
            var chillBreakfast:MutableList<SearchMeal> = mutableListOf()
            homeViewModel.ldtBreakfastMonanMeal().observe(viewLifecycleOwner, Observer {listMonAnMeal->
                chillBreakfast.addAll(listMonAnMeal)
                listParetChil1.add(ListDataParentChild(parentChildAdapter.TYPE_PARENT,parentBreakfast,null))
                listParetChil1.add(ListDataParentChild(parentChildAdapter.TYPE_CHILD,null,chillBreakfast))
                binding.rclBreakfast.layoutManager= LinearLayoutManager(requireContext())
                parentChildAdapter.setDataParetChild(requireContext(),listParetChil1)
                binding.rclBreakfast.adapter=parentChildAdapter
