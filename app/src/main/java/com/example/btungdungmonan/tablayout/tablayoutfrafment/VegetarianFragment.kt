...
import com.example.btungdungmonan.databinding.FragmentVegetarianBinding
...
class VegetarianFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        binding=FragmentVegetarianBinding.inflate(layoutInflater, container, false)
        return binding.root
    }
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        super.onViewCreated(view, savedInstanceState)
        homeViewModel.getCategoryMeal()
        homeViewModel.getVegetarianApi()
        getDessert()
    }
    private fun getDessert() {
        var listParetChil1:MutableList<ListDataParentChild> = mutableListOf()
        var parentVegetarian:MutableList<CategoriMeal> = mutableListOf()
        homeViewModel.ldtCategoryMealDaTa().observe(viewLifecycleOwner, Observer {listCategoriMeal->
            Log.d("AAAAAABBBBB",listCategoriMeal[11].strCategory)
            parentVegetarian.add(CategoriMeal("${listCategoriMeal[11].idCategory}","${listCategoriMeal[11].strCategory}","${listCategoriMeal[11].strCategoryDescription}","${listCategoriMeal[11].strCategoryThumb}"))
            var chillVegetarian:MutableList<SearchMeal> = mutableListOf()
            homeViewModel.ldtVegetarianMonanMeal().observe(viewLifecycleOwner, Observer {listMonAnMeal->
                chillVegetarian.addAll(listMonAnMeal)
                listParetChil1.add(ListDataParentChild(parentChildAdapter.TYPE_PARENT,parentVegetarian,null))
                listParetChil1.add(ListDataParentChild(parentChildAdapter.TYPE_CHILD,null,chillVegetarian))
                binding.rclVegetarian.layoutManager= LinearLayoutManager(requireContext())
                parentChildAdapter.setDataParetChild(requireContext(),listParetChil1)
                binding.rclVegetarian.adapter=parentChildAdapter
